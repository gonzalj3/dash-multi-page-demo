...
def get_layout():
    """Return the layout for app_4"""
    return html.Div([
        html.H1("App 4 - Append Approach"),
        html.P("This app appends graphs instead of recreating them."),
        html.Button("Click Me", id="my-button-app4"),
        html.Div(id="output-div-app4"),
        dcc.Store(id="graph-counter-app4", data=0)
    ])

def register_callbacks(app):
    """Register callbacks for app_4 with the provided app instance"""
    
    # Use the MATCH pattern to update individual graphs based on their corresponding toggle switch
    @app.callback(
        Output({'type': 'graph-app4', 'index': MATCH}, 'figure'),
        Input({'type': 'toggle-switch-app4', 'index': MATCH}, 'value'),
        State({'type': 'toggle-switch-app4', 'index': MATCH}, 'id')
    )
    def update_graph_app4(toggle_value, toggle_id):
    ...
    @app.callback(
        [Output("output-div-app4", "children"), Output("graph-counter-app4", "data")],
        Input("my-button-app4", "n_clicks"),
        [State("output-div-app4", "children"), State("graph-counter-app4", "data")]
    )
    def update_output_app4(n_clicks, current_children, current_count):
        ...
        if n_clicks > current_count:
            ...
            new_graph = html.Div([
                daq.ToggleSwitch(
                    id={'type': 'toggle-switch-app4', 'index': current_count},
                    label='Toggle Graph',
                    value=True  # Default to raw data
                ),
                dcc.Graph(
                    id={'type': 'graph-app4', 'index': current_count},
                    figure={
                        'data': raw_data_source,
